...
function LaverieCardSkeleton() {
    return (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm animate-pulse flex gap-3 p-3">
...
function EmptyState({ nbFiltresActifs, onClearFilters }: { nbFiltresActifs: number; onClearFilters: () => void }) {
    return (
        <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-base font-semibold text-slate-700 mb-1">Aucune laverie trouvée</h3>
...
function InitialState() {
    return (
        <div className="flex flex-col items-center justify-center py-14 text-center">
            <div className="w-16 h-16 bg-[#14A8DE]/10 rounded-full flex items-center justify-center mb-4">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#14A8DE" strokeWidth="1.5">
                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/>
                    <circle cx="12" cy="10" r="3"/>
                </svg>
            </div>
            <h3 className="text-base font-semibold text-slate-700 mb-1">Trouvez une laverie près de chez vous</h3>
            <p className="text-sm text-slate-400 max-w-xs">
                Autorisez la géolocalisation ou saisissez votre ville dans le champ de recherche.
            </p>
        </div>
    );
}
