...
export default function AjoutLaverie() {
    ...
    const addMachines = (n: number) =>
        set('machines', [...form.machines, ...Array.from({ length: n }, () => newMachine())]);
    ...
    const handleWiLineConnexion = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                setWiLineError(data?.message ?? 'Erreur lors de la récupération de la centrale.');
                return;
            }
            const centrale: WiLineCentrale = data;
            setWiLineConnecte(true);
            selectionnerCentrale(centrale);
        } catch {
