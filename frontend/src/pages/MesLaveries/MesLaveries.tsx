...
export default function MesLaveries() {
    ...
    return (
        <div className="max-w-[1280px] mx-auto bg-gray-50 pt-20 pb-16 px-5 lg:px-0">
            ...
            {laveries.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {laveries.map((laverie) => (
                        <div key={laverie.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
                            ...
                            <div className="mt-auto">
                                <Link 
                                    to={`/laveries/${laverie.id}`} 
                                    className="block w-full py-2.5 text-center text-sm font-medium text-[#14A8DE] bg-[#14A8DE]/10 hover:bg-[#14A8DE]/20 rounded-xl transition-colors"
                                >
                                    Voir la fiche
                                </Link>
                            </div>
